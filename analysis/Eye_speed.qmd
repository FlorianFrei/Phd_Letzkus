...
speed_folder <- "I:/Data/aligned/new/binfree/extra/Speed"
...
pupil_files <- list.files(
  "I:/Data/aligned/new/binfree/extra/Eye_dia",
  pattern = "\\.npy$",
  recursive = TRUE,
  full.names = TRUE
)
...
cam_delay <- read_csv_folder("I:/Data/aligned/new/binfree/extra/camttl") %>% rename('source' = file_origin)
...
pupils <- pupils %>%
  left_join(cam_delay, by = "source") %>% na.omit() %>% 
  group_by(source) %>%
  mutate(
    frame = row_number() ,
    time = (frame / 25) + camttl      # camera runs at 25 Hz
  )
...
sound <- data %>% separate_wider_delim(source, delim="_", names = c('Animal','condition'),cols_remove = F,too_many = "drop")
...
#write_csv(df_segments)
 data %>% distinct(source)
 cam_delay  %>% distinct(source)
 sound  <- sound %>% filter(source %in% aligned$source)
 aligned  %>% distinct(source)
 
 
 aligned %>% group_by(source) %>%  mutate( pupil = (pupil - mean(pupil)) / sd(pupil)) %>% ungroup() %>%  summarise(pupil = mean(pupil),.by=c(frame)) %>% ggplot(aes(frame,pupil))+
   geom_line()+
   xlim(0,900 *25)
 
gg <- aligned %>%  left_join(sound %>% unite("source", Animal,condition ) %>% select(source,continuous_start,trial_type))
```
...
df_segments <- bind_rows(df_segments)
df_segments <- df_segments %>% mutate(time_relative = round(time_relative,2))
...
df_segments %>% group_by(source) %>%  mutate(maxP = quantile(aligned$pupil, na.rm=T,0.99), minP = quantile(aligned$pupil, na.rm=T,0.01)) %>% filter(between(pupil,minP,maxP))  %>% mutate(pz = (pupil - mean(pupil))/sd(pupil)) %>% group_by(time_bin,stim_id) %>% summarise(pupil  = mean(pz)) %>% ungroup() %>% ggplot(aes(time_bin,pupil,col=stim_id))+
  geom_line()
...
df_segments %>% na.omit() %>% left_join(df_pre)  %>% na.omit() %>%  group_by(source) %>% mutate(df = (pupil - mean_pre) / sd_pre)  %>% group_by(time_relative,source) %>% summarise(df = mean(df)) %>% ggplot(aes(time_relative,df,col=source))+
  geom_line()+
  theme_minimal()+
  geom_smooth()

df_segments %>% na.omit() %>% left_join(df_pre) %>% na.omit() %>%  group_by(source) %>% mutate(df = (pupil - mean_pre) / sd_pre)  %>% group_by(time_relative) %>% summarise(df = mean(df)) %>% ggplot(aes(time_relative,df))+
  geom_line()+
   geom_smooth()



df_segments %>% na.omit() %>% left_join(df_pre) %>%  group_by(source) %>% mutate(df = (pupil - mean_pre) / sd_pre)  %>% group_by(time_relative,stim_id) %>% summarise(df = mean(df)) %>% ggplot(aes(time_relative,df,col=stim_id))+
  geom_line()+
  theme_minimal()


df_segments %>% filter(!stim_id %in% c('Opto_Upwsweep','Opto_Downsweep')) %>% na.omit() %>% left_join(df_pre) %>%  group_by(source) %>% mutate(df = (pupil - mean_pre) / sd_pre)  %>% group_by(time_relative,condition,stim_id) %>% summarise(df = mean(df)) %>% ggplot(aes(time_relative,df,col=condition))+
...
df_segments %>% na.omit() %>% left_join(df_pre)  %>% na.omit() %>%  group_by(source) %>% filter(source == "17_naive")


df_segments %>% filter(!stim_id %in% c('Opto_Upwsweep','Opto_Downsweep')) %>% na.omit() %>% left_join(df_pre) %>%  group_by(source) %>% mutate(df = (pupil - mean_pre) / sd_pre)  %>% group_by(time_relative,condition) %>% summarise(df = mean(df)) %>% ggplot(aes(time_relative,df,col=condition))+
  geom_smooth(method="loess",span=0.1,se=F)+
  theme_minimal()
ggsave("C:/Users/Freitag/Desktop/tcm/eye_recall.tiff",dpi=900)




df_segments %>% filter(condition == "recall") %>%  filter(!stim_id %in% c('Opto_Upwsweep','Opto_Downsweep')) %>% na.omit() %>% left_join(df_pre) %>%  group_by(source) %>% mutate(df = (pupil - mean_pre) / sd_pre) %>% separate_wider_delim(cols = source, names = c("Animal","condition2"),delim="_") %>% left_join(sd %>% mutate(Animal = as.character(Animal))) %>% separate_wider_delim(stim_id,delim='_', names=c('Opto','Stim_dir'), too_few='align_end')   %>% replace_na(list(Opto='no')) %>% mutate(Stim_dir = ifelse(Stim_dir =='Downsweep',Stim_dir,'Upsweep')) %>% mutate(fear = ifelse(Stim_dir == con_direction,"CS+","CS-")) %>%  group_by(time_relative,fear) %>% summarise(df = mean(df)) %>% ggplot(aes(time_relative,df,col=fear))+
  geom_smooth(method="loess",span=0.1,se=F)+
  theme_minimal()
ggsave("C:/Users/Freitag/Desktop/tcm/eye_css.tiff",dpi=900)
...
sd <- read_csv("H:/Data/aligned/extra/Shock_direction.csv") %>% mutate(con_direction = ifelse(direction =='Up','Upsweep','Downsweep')) %>% select(Animal = Animal2, con_direction) %>% mutate(Animal = as.character(Animal))
...
```{r}
df_segments %>% filter(between(time_relative,-1,6)) %>% na.omit() %>% mutate(time_relative = round(time_relative,1)) %>% mutate(speed = abs(speed))  %>% 
  group_by(time_relative)  %>% summarise(speed = mean(speed)) %>%   ggplot(aes(time_relative,speed))+
  geom_smooth(method='loess',span=0.1,se=F)+
  geom_vline(aes(xintercept=0), linetype = 'dashed',col='red')+
  theme_minimal()
ggsave("C:/Users/Freitag/Desktop/tcm/speed.tiff",dpi=900)

df_segments %>% separate_wider_delim(cols = source, names = c("Animal","condition2"),delim="_") %>% left_join(sd %>% mutate(Animal = as.character(Animal))) %>% separate_wider_delim(stim_id,delim='_', names=c('Opto','Stim_dir'), too_few='align_end')   %>% replace_na(list(Opto='no')) %>% mutate(Stim_dir = ifelse(Stim_dir =='Downsweep',Stim_dir,'Upsweep')) %>% mutate(fear = ifelse(Stim_dir == con_direction,"CS+","CS-"))%>% filter(between(time_relative,-1,6)) %>% na.omit() %>% mutate(time_relative = round(time_relative,1)) %>% mutate(speed = abs(speed))  %>% 
  group_by(fear,time_relative)  %>% summarise(speed = mean(speed)) %>%   ggplot(aes(time_relative,speed,col=fear))+
  geom_smooth(method='loess',span=0.1,se=F)+
  geom_vline(aes(xintercept=0), linetype = 'dashed',col='red')+
  theme_minimal()
ggsave("C:/Users/Freitag/Desktop/tcm/speedxfear.tiff",dpi=900)
```
