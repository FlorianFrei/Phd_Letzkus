...
# data <- load_data_with_brain_alignment(
#   datapath,
#   metapath,
#   depthpath,
#   structure_path
...
data_re<- Align_to_state(data_re, "Sound")
...
data <- all_loaded %>% mutate(file_id2 = file_id,file_id3 = file_id) %>%  unite('cluster_id',file_id,cluster_id) %>% unite('trial_number',file_id2,trial_number) %>% separate_wider_delim(file_id3, names = c('Animal','condition'),delim ='_')

data = rbind(data_re %>% mutate(cond = 'recall'), data_na %>% mutate(cond='naive')) %>% mutate(cond=) unite('cluster_id',file_id,cluster_id) %>% unite('trial_number',file_id2,trial_number)

data_na %>% filter(!cluster_id %in% data$cluster_id )
```
...
Trialless <- data  %>% 
  ...
  group_by(cond,trial_type, rel_time, cluster_id, n_trials, depth) %>%
...
g1 <- Trialless %>% filter(depth > 1500) %>% 
  filter(between(rel_time, -20, 8)) %>%
  group_by(cond,rel_time, trial_type) %>%
  summarise(Zscore = mean(Zscore)) %>%
  ungroup() %>% filter(Zscore < Inf)# %>% # group_by(cond,trial_type) %>% mutate(Zscore = Zscore - mean(Zscore)) %>% ungroup()
...
g1 <- g1 %>%
  group_by(trial_type) %>%
  mutate(smoothed1 = asymmetric_smooth(Zscore))
...
data %>% group_by(trial_type) %>% distinct(state_name)
...
g2 %>%
  ...
  scale_fill_gradient2(low = "darkblue", mid = "beige", high = "red", midpoint = 0, limits = c(round(min(g2$Zscore)), round(max(g2$Zscore) * 0.7)), breaks = c(round(min(g2$Zscore)), round(max(g2$Zscore) * 0.7)), na.value = "black") +
...
g2 %>%
  ...
  geom_tile() +
  scale_fill_gradient2(low = "blue", mid = "beige", high = "red", midpoint = , limits = c(round(min(g2$Zscore)), round(max(g2$Zscore) * 0.8)), breaks = c(round(min(g2$Zscore)), round(max(g2$Zscore) * 0.8)), na.value = "black") +
  labs(fill = "Z- scored FR") +
  theme_minimal() +
  scale_x_continuous(breaks = seq(-0.5, 6, by = 0.5)) +
  theme(axis.text.y = element_blank()) +
